...
<title>FilterByTimeWindow</title>
</head>
...
<hr>
<form id="filterByStop" action="Filter By Stop.jsp" method="get">
<table style="width:100%">
 ...
 ArrayList<Date> dpDates = (ArrayList<Date>)session.getAttribute("dpDates");
 ArrayList<Date> avDates = (ArrayList<Date>)session.getAttribute("avDates");
 int dpmin = Integer.valueOf(request.getParameter("dpMinTime"));  Date dpMinTime = dpDates.get(dpmin); 
 int dpmax = Integer.valueOf(request.getParameter("dpMaxTime"));  Date dpMaxTime = dpDates.get(dpmax);
 int avmin = Integer.valueOf(request.getParameter("avMinTime"));  Date avMinTime = avDates.get(avmin);
 int avmax = Integer.valueOf(request.getParameter("avMaxTime"));  Date avMaxTime = avDates.get(avmax);
 
 SimpleDateFormat sdf = new SimpleDateFormat("MM/dd HH:mm");
 String dpMinTimeStr = sdf.format(dpMinTime);
 String dpMaxTimeStr = sdf.format(dpMaxTime);
 String avMinTimeStr = sdf.format(avMinTime);
 String avMaxTimeStr = sdf.format(avMaxTime);
 
 String seat = (String)session.getAttribute("seat");
 ArrayList<Line> allokLine1 = (ArrayList<Line>) session.getAttribute("allokLine1");
 ArrayList<Line> allokLine2= (ArrayList<Line>) session.getAttribute("allokLine2"); 
 ArrayList<Line> filterByDATime1 = Filter.doFilterByDATime(allokLine1, dpMinTime, dpMaxTime, avMinTime, avMaxTime);
 ArrayList<Line> filterByDATime2 = Filter.doFilterByDATime(allokLine2, dpMinTime, dpMaxTime, avMinTime, avMaxTime);
 
 session.setAttribute("sortingLine1",filterByDATime1);
 session.setAttribute("sortingLine2",filterByDATime2);
 session.setAttribute("filteringLine1",filterByDATime1);
 session.setAttribute("filteringLine2",filterByDATime2);
 session.setAttribute("lineforConfirm1",filterByDATime1);
 session.setAttribute("lineforConfirm2",filterByDATime2); 
 session.setAttribute("dpMinTimeStr", dpMinTimeStr);
 session.setAttribute("dpMaxTimeStr", dpMaxTimeStr);
 session.setAttribute("avMinTimeStr", avMinTimeStr);
 session.setAttribute("avMaxTimeStr", avMaxTimeStr);
 %> 
 
 
 <%if((filterByDATime1.size()==0)&&(filterByDATime2.size()==0)){%>
...
//display flights with desired seat-class 
 if(filterByDATime1.size()!=0){
   for(int j=0; j<filterByDATime1.size(); j++){
	   ArrayList<Flight> fls = filterByDATime1.get(j).getFlights();
    %>
   <hr>
  ...
  <tr>
      ...
      <th><%=manageLineData.timeToString(filterByDATime1.get(j).getDuration())%></th>
      <th>Price:</th>
      <th><%=manageLineData.priceToString(filterByDATime1.get(j).getPrice())%></th>
  </tr> 
...
<td colspan="4" align="center">---------------------------------------
        Layover: <%=manageLineData.timeToString((filterByDATime1.get(j).getLayoverTime()).get(i))%>---------------------------------------</td></tr> <%} %>
<tr><td><br/></td></tr> 
   <%}//end inner for%>
...
<%if((filterByDATime2.size())!=0){%>
    <p align="center" style="font-size:20px"><strong>You May Also Consider Following Flights Without <%=seat%> Seat:</strong></p>
<% for(int j=0; j<filterByDATime2.size(); j++){
   ArrayList<Flight> fls = filterByDATime2.get(j).getFlights();   
 %>
   <hr>
   ...
   <tr>
      <th>Flight Duration:</th>
      <th><%=manageLineData.timeToString(filterByDATime2.get(j).getDuration())%></th>
      <th>Price:</th>
      <th><%=manageLineData.priceToString(filterByDATime2.get(j).getPrice())%></th>
   </tr> 
 <% for(int i=0; i<fls.size(); i++){ %>
		<tr>
			<td>Flight Number:</td>
      ...
      <tr>
       <% if((fls.size()!=1) && (i!=(fls.size()-1))){%>
          <td colspan="4" align="center">---------------------------------------
        Layover: <%=manageLineData.timeToString((filterByDATime2.get(j).getLayoverTime()).get(i))%>---------------------------------------</td>
      </tr> <%} %>
      <tr><td><br/></td></tr>
     <%}//end inner for%>
      <tr><td colspan="4" align="right"><input type="submit" value="Select"></td></tr>
