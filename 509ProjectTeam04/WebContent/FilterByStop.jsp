...
<table style="width:100%">
 ...
 ArrayList<Line> filteringLine1 = (ArrayList<Line>) session.getAttribute("filteringLine1");
 ArrayList<Line> filteringLine2= (ArrayList<Line>) session.getAttribute("filteringLine2"); 
 ArrayList<Line> filterByStop1 = Filter.doFilterByStop(filteringLine1, stopInfo);
 ArrayList<Line> filterByStop2 = Filter.doFilterByStop(filteringLine2, stopInfo);
 session.setAttribute("sortingLine1",filterByStop1);
 session.setAttribute("sortingLine2",filterByStop2);
 session.setAttribute("lineforConfirm1",filterByStop1);
 session.setAttribute("lineforConfirm2",filterByStop2);
/*  
...
for(int j=0; j<filterByStop1.size(); j++){
	   ArrayList<Flight> fls = filterByStop1.get(j).getFlights();
 %>
   <hr>
  ...
  <tr>
      ...
      <th><%=manageLineData.timeToString(filterByStop1.get(j).getDuration())%></th>
      <th>Price:</th>
      <th><%=manageLineData.priceToString(filterByStop1.get(j).getPrice())%></th>
  </tr> 
...
<td colspan="4" align="center">---------------------------------------
        Layover: <%=manageLineData.timeToString((filterByStop1.get(j).getLayoverTime()).get(i))%>---------------------------------------</td></tr> <%} %>
<tr><td><br/></td></tr> 
   <%}//end inner for%>
...
<% for(int j=0; j<filterByStop2.size(); j++){
 ...
 %>
   ...
   <tr>
      <th>Flight Duration:</th>
      <th><%=manageLineData.timeToString(filterByStop2.get(j).getDuration())%></th>
      <th>Price:</th>
      <th><%=manageLineData.priceToString(filterByStop2.get(j).getPrice())%></th>
   </tr> 
 <% for(int i=0; i<fls.size(); i++){ %>
		<tr>
			<td>Flight Number:</td>
      ...
      <tr>
       <% if((fls.size()!=1) && (i!=(fls.size()-1))){%>
          <td colspan="4" align="center">---------------------------------------
        Layover: <%=manageLineData.timeToString((filterByStop2.get(j).getLayoverTime()).get(i))%>---------------------------------------</td>
      </tr> <%} %>
      <tr><td><br/></td></tr>
     <%}//end inner for%>
      <tr><td colspan="4" align="right"><input type="submit" value="Select"></td></tr>
